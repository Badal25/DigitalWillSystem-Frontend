...
//       <h2 className="text-xl font-bold mb-4">Social Accounts</h2>
...
//           <label className="text-sm text-slate-300 mb-1">Nominee ID / Proof</label>
//           <input type="file" name="nominee_id_file" accept=".pdf,.jpg,.png" onChange={handleChange} />
//         </div>

//         <div className="flex items-center gap-2">
//           <label className="text-sm">Release to Nominee</label>
//           <input type="checkbox" name="release_to_nominee" checked={form.release_to_nominee} onChange={handleChange} />
//         </div>
...
//           <tbody>
//             {filtered.map((a) => (
//               <tr key={a.id} className="border-b border-slate-700 hover:bg-slate-800 transition">
//                 <td className="py-2 px-4">{a.platform}</td>
//                 <td className="py-2 px-4">{a.username}</td>
//                 <td className="py-2 px-4">{a.password}</td>
//                 <td className="py-2 px-4 truncate max-w-xs">{a.instructions}</td>
//                 <td className="py-2 px-4">{a.nominee_name}</td>
//                 <td className="py-2 px-4">{a.nominee_email}</td>
//                 <td className="py-2 px-4">
//                   {a.nominee_id_file ? (
//                     <a href={`http://localhost:5000/uploads/social_ids/${a.nominee_id_file}`} target="_blank" rel="noreferrer" className="text-blue-400 underline">View</a>
//                   ) : "-"}
//                 </td>
//                 <td className="py-2 px-4">{a.release_to_nominee ? "✅" : "❌"}</td>
//               </tr>
//             ))}
//           </tbody>
//         </table>
...
// const Input = ({ ...props }) => (
//   <input {...props} required className="bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-white" />
// );
  ...
  return (
    <motion.div className="p-6 bg-slate-900 rounded-2xl border border-slate-800 text-white">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <button className="col-span-3 bg-blue-600 py-2 rounded-lg hover:bg-blue-700 transition">Add Social Account</button>
      </form>

      {/* Search + Export */}
      <div className="flex justify-between mb-4">
        <input type="text" placeholder="Search platform / username" value={search} onChange={e => setSearch(e.target.value)}
          className="px-3 py-2 rounded-lg bg-slate-950 border border-slate-700 text-white" />
        <CSVLink data={filtered} filename="social_accounts.csv" className="px-3 py-2 bg-purple-600 rounded-lg hover:bg-purple-700">
          Export CSV
        </CSVLink>
      </div>
          ...
          <tbody>
            {filtered.map(a => (
              <tr key={a.id} className="border-b border-slate-700 hover:bg-slate-800 transition">
                ...
                <td className="py-2 px-4">
                  {a.nominee_id_file ? (
                    <a href={`http://localhost:5000/uploads/social_ids/${a.nominee_id_file}`} target="_blank" rel="noreferrer" className="text-blue-400 underline">
                      View
                    </a>
                  ) : "-"}
                </td>
