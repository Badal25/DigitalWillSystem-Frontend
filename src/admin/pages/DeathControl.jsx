...
export default function DeathControl() {
  ...
  const handleVerify = async (id) => {
    try {
      setProcessingId(id);
      const res = await verifyDocuments(id);
  ...
  const handleApprove = async (id) => {
    try {
      ...
      alert(res.data.message || "Case Approved");
      fetchCases();
    } catch (err) {
  ...
  const handleRelease = async (id) => {
    try {
      setProcessingId(id);
      const res = await releaseCase(id);
      alert(res.data.message || "Assets Released");
      fetchCases();
    } catch (err) {
  ...
  const handleReject = async (id) => {
    try {
      ...
      alert(res.data.message || "Case Rejected");
      fetchCases();
    } catch (err) {
                ...
                >
                  ...
                  <td>
                    {item.verification_document ? (
                      <a
                        href={`${BASE_URL}/${item.verification_document}`}
                        target="_blank"
                        rel="noreferrer"
